<template>
  <el-dialog :visible.sync="dialog" :close-on-click-modal="false" :before-close="cancel" :title="isAdd ? '新增菜单' : '编辑菜单'" append-to-body width="580px">
    <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="80px">
      ...
      <el-form-item :label="form.type.toString() === '2' ? '按钮名称' : '菜单名称'" prop="menuName">
        <el-input
      ...
      <el-form-item v-show="form.type.toString() !== '0'" prop="permission" label="权限标识">
      ...
      <el-form-item v-show="form.iframe === 'false' && form.type.toString() === '1'" prop="componentName" label="组件名称">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        permission: [{ min: 4, max: 16, message: '长度在 4 到 16 个字符', trigger: 'blur' }],
  ...
  methods: {
    ...
    submitForm() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          this.loading = true
          if (this.form.type.toString() === '2') {
            this.form.hidden = true // 新增按钮操作时隐藏菜单
          }
          if (this.isAdd) {
            this.doAdd()
          } else this.doEdit()
