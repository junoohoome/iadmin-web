...
watch(
  () => props.modelValue,
  (newVal) => {
    if (myEditor.value && newVal !== myEditor.value.getContent()) {
      myEditor.value.setContent(newVal)
    }
  }
)
...
function setContent(content: string) {
  myEditor.value?.setContent(content)
}

function getContent() {
  return myEditor.value?.getContent() || ''
}
...
function init() {
  ;(window as any).tinymce.init({
    ...
    setup: (editor: any) => {
      ...
      editor.on('init', () => {
        loading.value = true
        emit('on-ready') // 抛出 'on-ready' 事件钩子
